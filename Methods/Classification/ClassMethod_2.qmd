...
The result obtained shows that the unpruned model (SI @tbl-unpruned-result ) with a training error rate of 0.1385 had twelve terminal nodes (SI @fig-decisiontree-unpruned ) and retained the variables hpsa score and hpsa designation population in the model.
Meanwhile the pruned model (SI @fig-decisiontree-pruned ) using cross validation to obtain the number of trees with the minimum misclassification rate (SI @fig-misclass-pruned) had a slightly higher training error rate as shown in (SI @tbl-pruned-model ) cross validated model with eight terminal nodes.
...
We recommend the pruned tree (SI @fig-decisiontree-pruned ) model.
