...
Two decision tree models were employed to determine the predictors of hpsa status: An unpruned model and a pruned model based on implementing a 10-fold cross validation and selecting the tree size with the lowest misclassification rate and nodes.
...
class_2_tr <- tree(as.factor(hpsa_status) ~ ., data = train)
```
...
#| echo: false
#| warning: false
Yhat = predict(class_2_tr, newdata = test, type = "class")
```
...
set.seed(123)
cv <- cv.tree(class_2_tr, FUN = prune.misclass)
```
...
#| echo: false
set.seed(123)
trp <- prune.misclass(class_2_tr, best = 8)
```
...
#| include: false
#Unpruned Tree
yhat_unpruned = predict(class_2_tr, newdata = test, type = "class")
...
```{r}
#| tbl-cap: "Accuracy and Prediction Error Estimates Tree models"
#| echo: false
library(gt)
...
The result obtained shows that the unpruned model (SI @unpruned-result) with a training error rate of 0.1385 had twelve terminal nodes ([SI Table @fig-decisiontree-unpruned]) and retained the variables hpsa score and hpsa designation population in the model.
Meanwhile the pruned model ([SI Fig. @fig-decisiontree-pruned]) using cross validation to obtain the number of trees with the minimum misclassification rate ([SI Fig. @fig-misclass-pruned]) had a slightly higher training error rate as shown in ([SI Table @tbl-pruned-model]) cross validated model with eight terminal nodes.
...
We recommend the pruned tree ([SI Fig. @fig-decisiontree-pruned]) model.
