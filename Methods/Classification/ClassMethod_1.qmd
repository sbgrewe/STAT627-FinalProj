...
**Objective:** In the classification task, we seek to answer the question "Can health facilities within a county be designated as a Health Professional Shortage Area (HPSA) based on hpsa score, rural status, metropolitan indicator, designation type, HPSA designation population, HPSA population type and US-Mexico border indicator".

## Method 1: Logistic Regression

Two logistic regression models were employed to determine the predictors of hpsa status: A full model and a reduced model containing only predictors with p-values \<0.05.
We determined the best model by estimating the mean prediction error rate and using a 10-fold cross validation approach with a loss function.
We determined the best model by selecting one with the lowest mean prediction error rate
...
Giving the results obtained and from from the reduced and full logistic regression models and the results of model comparison we lack sufficient evidence (p = 2.446e-11) (SI @tbl-anova) to conclude that logistic regression (reduced) model is better than the logistic regression (full) model.
...
The output of the full logistic regression model is found at (SI @tbl-full-logistic).
